...
$(drivers): sps30-uart/sps_git_version.c FORCE
	cd $@ && $(MAKE) $(MFLAGS)

sps30-uart/sps_git_version.c: FORCE
...
$(release_drivers): sps30-uart/sps_git_version.c
	export rel=$@ && \
	...
	cp -r $${driver}/* "$${pkgdir}" && \
	perl -pi -e 's/^(.*)_dir :=.*$$/\1_dir := ./' "$${pkgdir}/Makefile" && \
